...
TEST(singleton, constructor) {
  int& a = fst::get_thread_local_instance<int>();
  fst::print("AAA", a);
  a = 32;
  fst::print("AAA", a);
...
TEST(singleton, constructor2) {
  using memory_buffer = std::array<std::uint8_t, 1024>;
  memory_buffer& mb = fst::get_thread_local_instance<memory_buffer>();
  //  using s_allocator = fst::stack_allocator<>;
  std::vector<int, fst::stack_allocator<int, 32>> ab;
