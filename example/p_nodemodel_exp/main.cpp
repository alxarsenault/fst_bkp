...
auto mul = [](const auto& lhs, const auto& rhs) {
	return lhs * rhs;
};
auto add = [](const auto& lhs, const auto& rhs) {
	return lhs + rhs;
};
auto return_two_values = [](const auto& lhs, const auto& rhs) {
...
>
node1<Computation, Output, std::tuple<Args...>>
make_node1(Computation c, Args... args) {
	return node1<Computation, Output, std::tuple<Args...>> { std::move(c),
			std::make_tuple(args...) };
}
...
struct adapter {
	...
	auto operator()(const std::tuple<Args1...>& in1_args
			, const std::tuple<Args2...>& in2_args)
	{
...
void lambda_storing_test() {
	...
	auto m_add = [](const auto& lhs, const auto& rhs) {
		return lhs + rhs;
	};
...
int main(/*int argc, char* argv[]*/) {
	...
	fst::print("adapter, (42 + 42) + (10 + 10) =",
			adapter_adds(std::make_tuple(42, 42), std::make_tuple(10, 10)));
