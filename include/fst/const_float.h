...
constexpr double atan2(const double y, const double x)
{
	return x > 0 ? atan(y / x)
			...
			: y < 0 && x < 0 ? atan(y / x) - M_PI
							 : y > 0 && x == 0 ? M_PI_2 : y < 0 && x == 0 ? -M_PI_2 : 0; // 0 == undefined
}
...
{
	return x >= 10.0 ? exponent_helper(x * 0.1, e + 1) : x < 1.0 ? exponent_helper(x * 10.0, e - 1) : e;
}

constexpr int exponent(const double x)
{
